...
var_data <-  dbGetQuery(con, 'SELECT *
                              FROM variant_occurrence v
                              LEFT OUTER JOIN Specimen sp on sp.specimen_id = v.specimen_id')

par1 <- "'%DIAGNOSIS%'"
par2 <- "'%_nte%'"
Data_surv <- dbGetQuery(con, paste0('select p.person_source_value, p.person_id, p.gender_source_value, p.race_source_value, cc.condition_status_source_value, cc.condition_source_value, vv.visit_start_datetime, ob.value_as_string, ob.observation_date, ob1.value_as_string, 
                              ob2.value_as_string, co.condition_start_datetime, co.condition_source_value
                              from person p
                              left outer join condition_occurrence cc on p.person_id = cc.person_id and cc.condition_type_concept_id = 44786627
                              left outer join visit_occurrence vv on p.person_id = vv.person_id and vv.visit_source_value like ', par1, 
                                    ' left outer join observation ob on p.person_id = ob.person_id and ob.observation_concept_id = 3009506
                              left outer join observation ob1 on p.person_id = ob1.person_id and ob1.observation_concept_id = 4174610
                              left outer join observation ob2 on p.person_id = ob2.person_id and ob2.observation_concept_id = 37397701
                              left outer join condition_occurrence co on p.person_id=co.person_id and co.condition_source_value like ', par2))

colnames(Data_surv) <- c('person_source_value', 'person_id', 'gender_source_value', "race_source_value", "condition_status_source_value", "condition_source_value",
                         'diagnosis', 'Vital_status', 'observation_date', 'tumor_status', 'treatment_outcome', 'new_tumor_event_date', 'new_tumor_event_type')
Data_surv$days_to_dia <- as.Date(Data_surv$observation_date) - as.Date(Data_surv$diagnosis)
Data_surv$daysnew_to_dia <- as.Date(Data_surv$new_tumor_event_date) - as.Date(Data_surv$diagnosis)

Data_surv$OS <- ifelse(Data_surv$Vital_status == "Dead", 1, 0)
Data_surv$OS_time <- Data_surv$days_to_dia

Data_surv$DSS <- ifelse(Data_surv$tumor_status == "TUMOR FREE", 0, 1)
Data_surv$DSS_time <- Data_surv$OS_time

Data_surv$daysnew_to_dia <- ifelse(Data_surv$daysnew_to_dia == "[Not Available]"|Data_surv$daysnew_to_dia == "[Not Applicable]", NA, as.character(Data_surv$daysnew_to_dia))
Data_surv$DFI <- ifelse(Data_surv$treatment_outcome != "Complete Remission/Response", NA, ifelse(is.na(Data_surv$daysnew_to_dia)==FALSE&Data_surv$new_tumor_event_type!="New Primary Tumor",1,0))
Data_surv$DFI_time <- ifelse(Data_surv$DFI == 1, as.numeric(as.character(Data_surv$daysnew_to_dia)), Data_surv$days_to_dia)

Data_surv$PFI <- ifelse(is.na(Data_surv$daysnew_to_dia)==FALSE|(Data_surv$tumor_status=="WITH TUMOR"&Data_surv$Vital_status=="Dead"), 1,0)
Data_surv$PFI_time <- ifelse(Data_surv$PFI == 1, ifelse(is.na(Data_surv$daysnew_to_dia)==FALSE, as.numeric(as.character(Data_surv$daysnew_to_dia)), as.numeric(as.character(Data_surv$days_to_dia))), Data_surv$days_to_dia)
Data_surv$gene <- NA
...
list_gene_all <- list_gene_all[order(sapply(list_gene_all, function(x) x))]
    ...
    dashboardBody(
      tabItems(
        ...
        tabItem(tabName = "SURVIVAL",
                ...
                selectInput(inputId = 'surv_gene', label = "Please select a gene", choices = list_gene_all, selected = "All")))),
...
server <- function(input, output, session) {
  ...
  output$path <- renderPlot({
    ...
    if (input$gene != 'All') {
      to_filt <- unique(Data_path[which(Data_path$target_gene_source_value == input$gene), 'pathway_source_value'])
  ...
  output$mut_gen <- renderPlot({
    ...
    if (input$pathway != 'All') {
      to_filt <- unique(Data_path[which(Data_path$pathway_source_value == 'Acute myeloid leukemia'), 'target_gene_source_value'])
  ...
  output$waterfall <- renderPlot({
    ...
    waterfall(inputData, fileType="Custom", variant_class_order = most_deleterious, plotMutBurden = TRUE,
              mainXlabel = FALSE, maxGenes=50, mainGrid = TRUE, mainLabelSize = 1,
  ...
  dd_surv <- reactive ({
    Data_surv$gene <- ifelse(Data_surv$person_id %in% var_data[which(var_data$target_gene_symbol==input$surv_gene), 'person_id'], 'mut', 'wt')
    return(Data_surv)})
  
  output$survival <- renderPlot({
    if (input$surv_gene == 'All') {
      ff <- paste0("Surv(" , input$endpoint, "_time,", input$endpoint, ") ~ 1")
    } else {
      ff <- paste0("Surv(" , input$endpoint, "_time,", input$endpoint, ") ~ gene")
    }
      km_fit <- surv_fit(as.formula(ff), data = dd_surv())
      ggsurv <- ggsurvplot(km_fit,  data = dd_surv(),  risk.table = TRUE, pval = TRUE, conf.int = TRUE, risk.table.y.text.col = TRUE, risk.table.col = 'black', font.main = c(18, "plain", "black"),
